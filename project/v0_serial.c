...
int main(int argc, char* argv[]) {
    ...
    printf("Execution Time (ms):Total = %.2fms, QKT = %.2fms, Softmax = %.2fms, AV = %.2fms\n", time_qkt + time_softmax + time_av, time_qkt, time_softmax, time_av);
    ...
    if (verify) {
        ...
        printf("Max absolute error between GPU and CPU: %.10f\n", max_diff);
        free(h_O_cpu);
    }
