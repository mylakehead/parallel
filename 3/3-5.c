...
void flip(unsigned char root, int *dims, int size, unsigned char array[100][2], int *rows) {
    shuffle_array(dims, size);

    for(int i=0; i<size; i++) {
        unsigned char f = root ^ (1 << dims[i]);
        array[*rows][0] = root;
        array[*rows][1] = f;
        (*rows)++;
...
void cube(unsigned char root, int *dims, int size, unsigned char array[100][2], int *rows) {
    // print_array(dims, size);
