...
__global__ void av_tiled_kernel(float* A, float* V, float* O, int B, int N, int D, int H) {
    ...
    float* O_h = O + (batch_idx * H + head_idx) * N * D_H;
...
int main(int argc, char* argv[]) {
    if (argc != 5) {
        printf("Usage: %s <Batch B> <Tokens N> <Dim D> <Heads H>\n", argv[0]);
        return 1;
    }
    ...
    if (D % H != 0) {
        printf("D must be divisible by H.\n"); return 1;
    }
    ...
    // verify
    float* O_concat = (float*)malloc(B * N * D * sizeof(float));
    for (int b = 0; b < B; ++b) {
        for (int n = 0; n < N; ++n) {
            for (int h = 0; h < H; ++h) {
                for (int d = 0; d < D / H; ++d) {
                    int dst = b * N * D + n * D + h * D_H + d;
                    int src = (b * H + h) * N * D_H + n * D_H + d;
                    O_concat[dst] = h_O[src];
                }
            }
        }
    }

    float* h_O_cpu = (float*)malloc(qkv_size);
    cpu_self_attention(h_Q, h_K, h_V, h_O_cpu, B, N, D, H);

    float max_diff = 0.0f;
    for (int i = 0; i < B * H * N * D_H; ++i) {
        float diff = fabs(O_concat[i] - h_O_cpu[i]);
